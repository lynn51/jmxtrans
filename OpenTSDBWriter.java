...
public class OpenTSDBWriter extends BaseOutputWriter {
    ...
    List<String> resultParser(Result result) throws UnknownHostException {
        ...
        } else if(subAttr !=null && !subAttr.isEmpty()) {//add by lyj, process CompositeData
        	for(String subAttributeName : subAttr) {
        		if (values.containsKey(subAttributeName)) {
                    String resultString = getResultString(className, attributeName, subAttributeName, (long)(result.getEpoch()/1000L), values.get(subAttributeName));
                    resultString = addTags(resultString);
                    if (getTypeNames().size() > 0) {
                        resultString = this.addTypeNamesTags(result, resultString);
                    }
        ...
        String getResultString(String className, String attributeName, String subAttributeName, long epoch, Object value) {
            String resultStringFormat = "put %s.%s.%s %d %s";
            return String.format(resultStringFormat, className, attributeName, subAttributeName, epoch, value);
        }
    ...
    protected String    addTypeNamesTags (Result result, String resultString) {
        ...
        if ( mergeTypeNamesTags ) {
            ...
            retVal = addTag(retVal, StringUtils.join(getTypeNames(), ""), getConcatedTypeNameValues(result.getTypeName()).replaceAll("\"", ""));
        }
        else {
    ...
    public void start() throws LifecycleException {
        ...
        subAttr = (List<String>)this.getSettings().get("subAttr");
